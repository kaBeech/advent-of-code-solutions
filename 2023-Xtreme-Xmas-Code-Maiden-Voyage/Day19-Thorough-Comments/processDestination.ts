...
import { Category, EvaluationResult, Part, Rule, Workflow } from "./types.ts";
...
export default (
  ...
  switch (destination) {
    case `A`:
      ...
      // console.count();
      return { category, value, passes: true, ruleStack };
    case `R`:
      // If the destination is Rejected, return false so that processing is halted.

      return { category, value, passes: false, ruleStack };
    default:
