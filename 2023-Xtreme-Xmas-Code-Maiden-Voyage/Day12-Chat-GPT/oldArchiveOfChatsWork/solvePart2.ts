...
function parseInput(inputString: string): BoxAndItemsRecord[] {
  ...
  for (const row of rows) {
    let [boxData, itemsList] = row.split(" ");
    const boxDataExtender = `?` + boxData;
    const itemsListExtender = `,${itemsList}`;
    boxData = boxData + boxDataExtender.repeat(4);
    itemsList = itemsList + itemsListExtender.repeat(4);
    console.log(boxData, itemsList);
...
function findNumberOfPossibleArrangements(
  ...
  for (const sequence of potentialSequences) {
    ...
    if (!hasLowerIdInEarlierSequence && !hasHigherIdInLaterSequence) {
      ...
      if (numberOfPossibleArrangements > 0) {
...
function main(): number {
  ...
  for (const record of parsedRecords) {
    ...
    console.log(totalNumberOfPossibleArrangements);
  }
