...
solvePart input =
  let equationIsSolvable (result, operands) = any (tryEquation 0 (result : operands)) (genOperators (length operands - 2))
   ...
   in sum (map fst (filter (equationIsSolvable) (map (\line -> (read (head (words (map (\c -> if c == ':' then ' ' else c) line))), map read (tail (words (map (\c -> if c == ':' then ' ' else c) line))))) (lines input))))
