...
solvePart input =
  let genOperators n = if n == 0 then [[op] | op <- "*+|"] else [(operator : rest) | rest <- genOperators (n - 1), operator <- "*+|"]
   ...
   in sum (map fst (filter ((\equation -> any (tryEquation 0 (uncurry (:) equation)) (genOperators (length (snd equation) - 2)))) (map (\line -> (read (head (words (map (\c -> if c == ':' then ' ' else c) line))), map read (tail (words (map (\c -> if c == ':' then ' ' else c) line))))) (lines input))))
